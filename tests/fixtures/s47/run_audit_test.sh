...
python3 "${REPO_ROOT}/src/python_scripts/audit_normative_projection.py" --help > /dev/null 2>&1
check "--help exits 0" "$?"

# Test 2: Audit mode on fixture (no writes)
echo "[s47-test] Test 2: --mode audit on fixture"
python3 "${REPO_ROOT}/src/python_scripts/audit_normative_projection.py" \
    --mode audit \
    --input-root "${FIXTURES}" \
