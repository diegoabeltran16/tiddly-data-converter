...
#
# Usage:
#   bash src/shell_scripts/baseline_regen.sh
#
# Prerequisites:
#   - Go 1.24+ installed
...
go build -o "${REPO_ROOT}/src/go/canon/accumulate" ./cmd/accumulate

echo "[baseline_regen] Generating golden baseline from ${INPUT_DIR}..."
mkdir -p "${GOLDEN_DIR}"

"${REPO_ROOT}/src/go/canon/accumulate" \
  --input "${INPUT_DIR}" \
  --out "${SNAPSHOT_OUT}" \
...
rm -f "${REPO_ROOT}/src/go/canon/accumulate"
