...
def module_to_repo_path(module: str, source_path: Path, root: Path) -> str | None:
    ...
    if len(parts) == 1:
        candidates.extend([
            root / "src" / "python_scripts" / f"{parts[0]}.py",
            root / "src" / "python_scripts" / parts[0] / "__init__.py",
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                if not target:
                    continue
                kind = "test_imports_subject" if rel_source.startswith("tests/") and target.startswith("src/python_scripts/") else "python_ast_import"
