...
def run_reconstruction_gate(
    ...
    *,
    ...
    result = run_command(args, cwd=REPO_ROOT / "src" / "rust" / "doctor")
...
def option_preparation() -> None:
    ...
    checks = [
        ...
        ("python_scripts", (REPO_ROOT / "src" / "python_scripts").exists(), "python_scripts"),
        ("admit_session_candidates.py", (REPO_ROOT / "src" / "python_scripts" / "admit_session_candidates.py").exists(), ""),
        ("src/go/canon", (REPO_ROOT / "src" / "go" / "canon").exists(), "src/go/canon"),
        ("src/go/bridge", (REPO_ROOT / "src" / "go" / "bridge").exists(), "src/go/bridge"),
        ("src/go/ingesta", (REPO_ROOT / "src" / "go" / "ingesta").exists(), "src/go/ingesta"),
    ...
    if shutil.which("cargo") and (REPO_ROOT / "src" / "rust" / "doctor").exists():
        ...
        print("- rust kernel perimeter: FALTA - cargo o src/rust/doctor no disponible")
...
def option_modal_delta_staging() -> None:
    ...
    result = run_command(
        [
            sys.executable,
            "src/python_scripts/stage_modal_delta.py",
            "--run-id",
            run_id,
            "--run-gates",
        ],
...
def option_repository_exporter() -> None:
    ...
    runner = REPO_ROOT / "src" / "python_scripts" / "rep_export_LINUXandMAC" / "run_export_menu.py"
    if not runner.exists():
        tdc_cat_error(f"No se encontro el runner: {display(runner)}")
        return

    env = os.environ.copy()
    python_scripts_root = REPO_ROOT / "src" / "python_scripts"
