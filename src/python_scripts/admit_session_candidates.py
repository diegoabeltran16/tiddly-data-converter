...
def _run_normalize(lines: list[dict[str, Any]], work_dir: Path) -> tuple[list[dict[str, Any]], CommandResult]:
    ...
    result = _run_command(cmd, cwd=REPO_ROOT / "src" / "go" / "canon", env=_go_env())
...
def _run_test_gates(skip_tests: bool) -> tuple[dict[str, str], list[CommandResult]]:
    ...
    go_canon = _run_command(["go", "test", "./...", "-count=1"], cwd=REPO_ROOT / "src" / "go" / "canon", env=_go_env())
    results.append(go_canon)

    go_bridge = _run_command(["go", "test", "./...", "-count=1"], cwd=REPO_ROOT / "src" / "go" / "bridge", env=_go_env())
...
def _run_canon_proposal_validate(candidate_file: Path, canon_dir: Path) -> CommandResult:
    return _run_command(
        [
            sys.executable,
            "src/python_scripts/canon_proposal.py",
            "validate",
            "--proposal-file",
            str(candidate_file),
            "--canon-dir",
