# 🧪🧱 Hipótesis
`## 🧪🧱 Hipótesis` funciona como el **núcleo madre** que orienta el registro y tratamiento de hipótesis dentro del sistema. No demuestra ni valida por sí mismo una idea: conserva formulaciones tentativas sin confundirlas con hechos, definiciones o resultados consolidados.

## Propósito
- `rol_principal`: `procedimiento`.
- `rol_secundario`: `definición`.
- Admitir pensamiento provisional de forma trazable y gobernada.
- Distinguir con claridad entre formulación tentativa, evidencia, interpretación y definición.
- Conservar líneas de exploración que todavía no deben consolidarse.

## Cuándo aplica
- Cuando aparece una conjetura, supuesto o línea de validación pendiente.
- Cuando una sesión produce una formulación todavía no estabilizada.
- Cuando una intuición inicial necesita conservarse sin promoverse a hecho.
- Cuando una hipótesis cambia de estatuto por confirmación, refinamiento, contradicción o descarte.

## Obligaciones
- Formular cada hipótesis con claridad suficiente para volver a evaluarla.
- Separar hipótesis de hechos, resultados y definiciones.
- Registrar contexto de surgimiento, alcance y relación con el tema.
- Conservar las líneas de validación relevantes.
- Registrar confirmación, refinamiento, contradicción o descarte cuando ocurra.
- Usar `# 3_🧪 Hipótesis inicial` y `#### 🌀🧪 Hipótesis de sesión ##` cuando la hipótesis pertenezca a esas escalas.
- Remitir a `## 🧾🧱 Procedencia epistemológica` cuando haga falta explicar el origen o soporte de una hipótesis.

## No hacer
- No presentar una hipótesis como conclusión consolidada.
- No esconder incertidumbre detrás de redacción categórica.
- No inflar esta capa con ocurrencias sin función operativa.
- No usar este nodo como sustituto del desarrollo temático o de la procedencia.

## Regla transversal S66
- Toda sesión debe producir hipótesis de sesión en `data/sessions/03_hipotesis/<session>.md.json`, aunque sea para declarar que no se abrieron hipótesis nuevas.
- El `title` de hipótesis de sesión debe usar `#### 🌀🧪 Hipótesis de sesión <NN> = <slug>`, por ejemplo `#### 🌀🧪 Hipótesis de sesión 69 = canon-admission-hardening-and-docs-v0`.
- Una hipótesis de sesión candidata al canon debe conservar su estatuto tentativo y no reclamar autoridad final antes de validación local y reversibilidad.
- El diagnóstico de sesión debe registrar si las hipótesis quedaron validadas, abiertas, descartadas o pendientes.

## Interacción con otros nodos
- Requiere `## 🎯🧱 Detalles del tema` para que la hipótesis quede situada dentro de un marco reconocible.
- Usa `## 🧾🧱 Procedencia epistemológica` para explicar origen y soporte contextual cuando sea necesario.
- Usa `## 🌀🧱 Desarrollo y Evolución` para conservar la vida histórica de cada hipótesis.
- Usa `## 🧭🧱 Protocolo de Sesión` cuando la formulación, revisión o tensión ocurra dentro de una sesión concreta.
- Puede apoyarse en `## 📚🧱 Glosario y Convenciones` cuando la hipótesis depende de distinciones terminológicas finas.

## Criterio de salida
- Debe poder distinguirse qué sigue siendo tentativo y qué ya no.
- Debe quedar visible el alcance de la hipótesis, su contexto y su línea de validación.
- Un agente debe poder revisar después si la hipótesis se confirmó, se refinó, se contradijo o se descartó.

La hipótesis no reemplaza el desarrollo del tema, pero evita que sus direcciones tentativas se pierdan o se mezclen con otros tipos de conocimiento.
